...
function isDisallowedDiscoveryQuerySource(value: unknown): boolean {
  const normalized = String(value || '').trim().toLowerCase();
  if (!normalized) return true;
  if (normalized.startsWith('cache_')) return true;
  if (normalized.includes('fallback')) return true;
  if (normalized.includes('invoke_outer_cache_guard')) return true;
  return false;
}

function mergeUniqueProductsByIdentity(items: ProductResponse[]): ProductResponse[] {
  const seen = new Map<string, ProductResponse>();
  for (const item of items) {
    const merchantId = String(item?.merchant_id || '').trim();
    const productId = String(item?.product_id || '').trim();
    if (!productId) continue;
    seen.set(`${merchantId}:${productId}`, item);
  }
...
export async function getDiscoveryProducts(
  ...
  const neededCount = offset + clampedLimit;
  const perQueryLimit = Math.max(2, Math.min(4, Math.ceil(neededCount / DISCOVERY_QUERY_PACK.length)));

  const settled = await Promise.allSettled(
    DISCOVERY_QUERY_PACK.map((query) =>
      callGatewayWithTimeout(
        {
          operation: 'find_products_multi',
          payload: {
            search: {
          ...
          metadata: {
            source: 'shopping',
            ui_surface: 'discovery',
            discovery_feed: 'homepage_hot_deals',
          },
        },
        8000,
      ),
  ...
  const merged = mergeUniqueProductsByIdentity(
    settled.flatMap((result) => {
      if (result.status !== 'fulfilled') return [];
      const metadata =
        result.value && typeof result.value === 'object' && typeof (result.value as any).metadata === 'object'
          ? ((result.value as any).metadata as Record<string, unknown>)
          : {};
      if (isDisallowedDiscoveryQuerySource(metadata?.query_source)) return [];
      const products = Array.isArray((result.value as any)?.products) ? (result.value as any).products : [];
      return products.map((product: RealAPIProduct | ProductResponse) => normalizeProduct(product));
    }),
  );

  return merged.slice(offset, offset + clampedLimit);
}
