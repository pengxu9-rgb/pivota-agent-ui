...
if (!_PHOTO_SKIN_AGENT_ENVS.some((name) => process.env[name])) {
  warnIfHardcodedFallbackUsed({
    routeLabel: 'api/photo-analysis/skin',
    envVarsTried: _PHOTO_SKIN_AGENT_ENVS,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const upstream = await fetch(`${resolveAgentBaseUrl()}/v1/analysis/skin`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      {
        status: 'failed',
        error: 'SKIN_ANALYSIS_PROXY_FAILED',
