...
function splitSetCookieHeader(value: string): string[] {
  ...
  if (!v) return [];
  // Undici may concatenate multiple `set-cookie` headers into a single string.
  // Our cookies do not include `Expires=...`, so this split is safe here.
  return v
    .split(/,(?=[^;]+?=)/g)
    .map((part) => part.trim())
    .filter(Boolean);
...
export async function GET(req: NextRequest, { params }: RouteContext) {
  ...
  return proxy(req, { upstreamBase: getUpstreamAccountsBase(), path });
}
