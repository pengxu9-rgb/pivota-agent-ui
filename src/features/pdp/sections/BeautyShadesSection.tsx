...
export function BeautyShadesSection({
  ...
  const undertone =
    selectedVariant.beauty_meta?.undertone || getOptionValue(selectedVariant, ['undertone', 'tone']);
    ...
    <div className="py-6">
        ...
        <div className="px-4 py-3 border-b border-border">
          ...
          <div className="flex flex-wrap gap-1.5">
            {popularLooks.map((tag) => (
              <button
                key={tag}
                className="px-2.5 py-1 rounded-full border border-border bg-card text-xs hover:border-primary/50"
              >
                {tag}
              </button>
            ))}
      ...
      <div className="px-4 py-3 border-b border-border">
        ...
        <p className="mt-1.5 text-xs text-muted-foreground">
          Share how this shade looks on you to help others choose.
        </p>
      </div>
        ...
        <div className="px-4 py-3 bg-muted/30">
          ...
          {importantInfo.length ? (
            ...
            <p className="text-xs text-muted-foreground">
              No additional safety or usage details provided.
            </p>
          )}
        </div>
      ) : null}
      ...
      {galleryItems.length ? (
        <div className="px-4 py-3">
          ...
          {brandName ? (
            <div className="text-xl font-serif tracking-widest text-center mb-4">
