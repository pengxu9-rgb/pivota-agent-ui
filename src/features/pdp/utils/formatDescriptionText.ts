...
function insertHeadingBreaks(input: string): string {
  ...
  // Longest first (e.g. KEY INGREDIENTS before INGREDIENTS) to reduce overlaps.
  ...
  for (const heading of headings) {
    ...
    text = text.replace(re, (match, offset: number) => {
      ...
      const okAfter = after == null || after === '\n' || after === ':' || (after >= 'A' && after <= 'Z');
...
function insertLabelLineBreaks(input: string): string {
  // Break lines before "Label: ..." patterns (Function:, Recommended Use:, Key Ingredients:, etc.).
  // Keeps this conservative by requiring a colon and title-case-ish prefix.
...
export function formatDescriptionText(input: unknown): string {
  ...
  // Many external descriptions lose structural breaks; re-introduce common section headings.
